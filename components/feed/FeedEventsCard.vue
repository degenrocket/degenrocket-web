  ...
  <div class="p-1 py-2 border-b border-borderColor-light dark:border-borderColor-dark"
    ...
    <div class="">
      ...
      <span v-if="post.authors?.[0].addresses?.[0].value" class="">
        ...
        >
          <ExtraBlockies :seed="String(post.authors?.[0].addresses?.[0].value)" :scale="2" class="inline-block" />

          <!-- client-only tags solve hydration mismatch warning -->
          <client-only>
            <span
              v-if="getMetadataByAddressNostr(String(post.authors?.[0].addresses?.[0].value), 'username') && getMetadataByAddressNostr(String(post.authors?.[0].addresses?.[0].value), 'username') !== 'none'"
              class="ml-1"
            >
              <span class="">
                {{ getMetadataByAddressNostr(String(post.authors?.[0].addresses?.[0].value), 'username').slice(0,40) }}
              </span>
              <span class="text-sm">
                (Nostr)
              </span>
          ...
          <span v-else class="ml-1" >
            <span v-if="enableAutoGeneratedNames">
              {{autoGeneratedName(post.authors?.[0].addresses?.[0].value)}}
              ({{sliceAddress(String(post.authors?.[0].addresses?.[0].value), 6)}})
            </span>
            <span v-else>
              {{sliceAddress(String(post.authors?.[0].addresses?.[0].value), 6)}}
            </span>
    ...
    <!--
    <div v-if="!post.signatures?.[0].value">
      <nuxt-link
        v-if="post.title"
        :to="`/news/${post.ids?.[0].value}`"
        class="nuxt-link"
      >
    ...
    <div v-if="post.ids?.[0].value">
      <!-- short URL -->
      <nuxt-link
        v-if="post.title && enableShortUrlsForWeb3Actions"
        :to="`/news/${String(post.ids?.[0].value).slice(0,shortUrlsLengthOfWeb3Ids)}`"
        class="nuxt-link"
      >
      ...
      <!-- short URL -->
      <nuxt-link
        v-if="!post.title && post.content && enableShortUrlsForWeb3Actions"
        :to="`/news/${String(post.ids?.[0].value).slice(0,shortUrlsLengthOfWeb3Ids)}`"
        class="nuxt-link"
      >
        <span @click="postClicked()">{{post.content.slice(0, 100)}}</span>
      </nuxt-link>
      <!-- full URL -->
      <nuxt-link
        v-if="!post.title && post.content && !enableShortUrlsForWeb3Actions"
    ...
    <div>
      ...
      <span v-if="spasm.getTotalOfReaction(post, 'bearish')" class="mr-2 text-colorNotImportant-light dark:text-colorNotImportant-dark">
