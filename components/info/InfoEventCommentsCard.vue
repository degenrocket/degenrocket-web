    ...
    <div class="overflow-auto overflow-wrap break-words">
      <div class="text-colorNotImportant-light dark:text-colorNotImportant-dark">
        <span v-if="comment.authors?.[0].addresses?.[0].value?.toString()">
          ...
          <ExtraBlockies :seed="extractOneAuthorAddressForDisplay(comment, true)" :scale="2" class="inline-block mr-1" />
          ...
          <client-only>
            <span
              v-if="getMetadataByAddressNostr(extractOneAuthorAddressForDisplay(comment, true), 'username') && getMetadataByAddressNostr(extractOneAuthorAddressForDisplay(comment, true), 'username') !== 'none'"
              class="ml-1"
            >
              <span class="">
                {{ getMetadataByAddressNostr(extractOneAuthorAddressForDisplay(comment, true), 'username').slice(0,40) }}
              </span>
              <span class="text-sm">
                (Nostr)
              </span>
            ...
            <span v-else class="ml-1" >
              {{ extractOneAuthorAddressForDisplay(comment) }}
            </span>
          </client-only>

          </nuxt-link>
          <ExtraAddressIcons
            v-if="extractOneAuthorAddressForDisplay(comment, true)"
            :key="extractOneAuthorAddressForDisplay(comment, true)"
            :value="extractOneAuthorAddressForDisplay(comment, true)"
