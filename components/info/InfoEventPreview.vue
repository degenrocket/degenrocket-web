<template>
  <div v-if="event" class="overflow-auto overflow-wrap break-words">
    ...
    <div v-if="event.authors?.[0]?.addresses?.[0]?.value" class="text-base text-colorNotImportant-light dark:text-colorNotImportant-dark">
      <span>Author: </span>
      <ExtraBlockies
        :seed="event.authors?.[0]?.addresses?.[0]?.value?.toString()"
        :scale="2"
        class="inline-block mr-1"
      />
      <nuxt-link :to="`/authors/${event.authors?.[0]?.addresses?.[0]?.value?.toString()}`"
        ...
        <client-only>
            <span
              v-if="getMetadataByAddressNostr(event.authors?.[0]?.addresses?.[0]?.value?.toString(), 'username') && getMetadataByAddressNostr(event.authors?.[0]?.addresses?.[0]?.value?.toString(), 'username') !== 'none'"
              class="ml-1"
            >
              <span class="">
                {{ getMetadataByAddressNostr(event.authors?.[0]?.addresses?.[0]?.value?.toString(), 'username').slice(0,40) }}
              </span>
              <span class="text-sm">
                (Nostr)
              </span>
          ...
          <span v-else class="">
            <span v-if="enableAutoGeneratedNames">
              {{autoGeneratedName(event.authors?.[0]?.addresses?.[0]?.value?.toString())}}
              ({{event.authors?.[0]?.addresses?.[0]?.value?.toString().slice(0, 6)}}...{{event.authors?.[0]?.addresses?.[0]?.value?.toString().slice(-4)}})
            </span>
            <span v-else>
              {{event.authors?.[0]?.addresses?.[0]?.value?.toString().slice(0, 6)}}...{{event.authors?.[0]?.addresses?.[0]?.value?.toString().slice(-4)}}
            </span>
      ...
      <ExtraAddressIcons
        v-if="event.authors?.[0]?.addresses?.[0]?.value?.toString()"
        :key="event.authors?.[0]?.addresses?.[0]?.value?.toString()"
        :value="event.authors?.[0]?.addresses?.[0]?.value?.toString()"
